...
MD = os.path.join(os.path.dirname(__file__), "据理力争_参考答案_imao_L12.md")
TITLE = "据理力争小课堂 · 第12课费曼参考答案（当众被批评怎么稳·稳听分应收进）"

def call(apipath, body):
    p = subprocess.run([NODE, CJS, apipath, json.dumps(body, ensure_ascii=False)],
                       capture_output=True, text=True, encoding="utf-8")
    if p.returncode != 0:
        print("ERR stderr:", p.stderr[:800]); sys.exit(1)
