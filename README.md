...
## 课程结构（每课一个永久网页，永不更新）
- 根目录 `index.html` = 课程目录（hub），点击进入各课。
- `lessons/lesson_XX/index.html` = 第 XX 课独立永久页（自带 `content.js` 与 `audio/`）。
...
`https://de333334.github.io/xiaoxiao-classroom/`

发布新的一课时，把 `lessons/lesson_XX/` 生成好，更新根目录 `index.html` 的卡片，
然后 `git add -A && git commit && git push` 即可。每课网页生成后不再改动（永久）。
